...
public class ConsistentHashRouter<T extends Node> {
    ...
    // 해시공간 계산 추가
    public Long calculateHashSpaceOfNode(T pNode) {
        // 가상 노드가 있는 경우 할당된 해시공간이 분산되어 있음
        long sum = 0L;
        for (VirtualNode<T> vNode : ring.values()) {
            if (vNode.isVirtualNodeOf(pNode)) {
                Long vNodeHash = hashFunction.hash(vNode.getKey());
                SortedMap<Long, VirtualNode<T>> headMap = ring.headMap(vNodeHash);
                if(!headMap.isEmpty()){
                    sum += vNodeHash - headMap.lastKey();
                }else{
                    sum += vNodeHash + (Long.MAX_VALUE - ring.lastKey());
                }
            }
        }
        return sum;
